...
FR-11 カスタムボタン設定（カード内インライン編集に統一）
- 入力カード右上の編集トグルで編集モード切替（✎）。カード内で直接ラベルと秒数を編集
- 設定項目: ラベル文字列と移動秒数のペア
...
- 保存: `chrome.storage.local`（フォールバックで `localStorage`）に `'custom-buttons'` 配列を保存
...
- ボタンクリック: ポータル方式の編集ポップアップを document.body 直下に表示（Shadow DOM 上でもz-index問題なし）
- フィールド: 「表示ラベル（英数字, +, -, 12文字まで）」「移動秒数」
- ボタン: 「保存」「キャンセル」（多言語対応）
- 操作: Enterキーで保存、Escキーでキャンセル
- 状態管理: 操作パネルを閉じると編集モードとポップアップを自動リセット
